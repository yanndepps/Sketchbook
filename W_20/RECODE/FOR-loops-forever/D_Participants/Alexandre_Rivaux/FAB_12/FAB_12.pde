...
void setup()
{
  ...
  randomPeriods = new float[cols][rows];
  
  for (int j = 0; j<cols; j++)
  {
    for (int i = 0; i<rows; i++)
    {
    int orientation = (int) random(5);
    float dphi = random(10);
      orientationList[j][i] = orientation;
      randomPeriods[j][i] = dphi;
    }
  }
}
...
void draw()
{
  ...
  { 
    ...
    for (int i = 0; i<rows; i++)
    {
      ...
      float sw = map(sin(theta)*phiAmplitude, -phiAmplitude, phiAmplitude, 0, 2);
      float alpha = map(sin(theta)*phiAmplitude, -phiAmplitude, phiAmplitude, 0, 100);
