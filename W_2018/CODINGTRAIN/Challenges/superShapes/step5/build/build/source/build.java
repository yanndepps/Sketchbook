...
public void draw() {
	...
	for (int i = 0; i < total+1; i++) {
		...
		float r2 = supershape(lat, m, 0.2f, 1.7f, 1.7f);	  			// radius for the first supershape based on latitude
		for (int j = 0; j < total+1; j++) {
			float lon = map(j, 0, total, -PI, PI);
			float r1 = supershape(lon, m, 0.2f, 1.7f, 1.7f);			// radius for the second supershape based on longitude
